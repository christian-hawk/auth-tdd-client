...
class TestPreselectedProvider(FlaskBaseTestCase):
    ...
    def test_config_should_have_preselected_provider_option(self):
        self.assertTrue(
            hasattr(clientapp.cfg, 'PRE_SELECTED_PROVIDER'),
    ...
    def test_if_authlib_authorize_redirect_uri_contains_preselected_ext_prov(self):
        authorize_redirect_response = clientapp.oauth.op.authorize_redirect(
            clientapp.cfg.REDIRECT_URIS[0],
            preselectedExternalProvider='foo'
        )

        self.assertTrue(
            'preselectedExternalProvider' in authorize_redirect_response.location,
    ...
    def test_redirect_response_url_should_have_preselected_provider_param(self):
    ...
    def test_preselected_provider_id_should_exist_in_cfg(self):
        self.assertTrue(
            hasattr(clientapp.cfg, 'PRE_SELECTED_PROVIDER_ID')
        )
    ...
    def test_get_selected_provider_should_return_base64(self):
        ...
        self.assertEqual(
            clientapp.get_preselected_provider(),
            expected_response
        )
    ...
    def test_url_should_contain_selected_provider_base64(self):
        ...
        response = self.client.get(url_for('login'))
        self.assertTrue(
            'preselectedExternalProvider=%s' % expected_base_64 in response.location,
            'No preselectedExternalProvider=%s in /login response' % expected_base_64
        )
